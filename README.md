...
- 重写跨页脚注流程：改为由 Agent 结合分页 Markdown 与原图逐页判断、直接写回原文件，不再调用合并脚本或生成页码范围文件；同时允许依据清晰原图忠实补录 OCR 漏失的脚注续文。
- 增强 OCR 规则：无图注的分隔插图按空页处理，装饰性分隔符直接忽略，并补充脚注标号识别、法语续页提示与禁止嵌套 `<sup>` 的要求。
- 更新 EPUB 数学公式导出参数为 `--math-method=webtex:https://latex.codecogs.com/svg.latex?`，保留 `\cancel` 支持并避免 Pandoc 弃用警告。
- 支持长脚注按句拆段，确保每段使用独立且完整的 `<sup>...</sup>` 标签包裹。
- 精简排版成书和 Calibre 元数据写入后的默认验证步骤，避免在命令成功后进行不必要的重复检查；同时说明未指定丛书序号时 Calibre 显示 `#1` 的含义。

### 2.2.0

- 重组 `pdf-set` 处理流程：支持从 OCR 分页直接排版成书，并将标题分类调整到排版之后，移除已废弃的粗合并脚本与说明。
- 新增跨页脚注修复流程，包含专用 prompt、批量处理脚本、本地验证和失败重试。
- 新增图片占位符替换指南与边界检查脚本，用于定位、裁切并验证书中插图。
- 新增默认封面生成和 Calibre EPUB 元信息写入流程。
- 增强 OCR：支持并发批处理、页面范围、断点续传、单图模式和模型回退。
- 新增快速翻译脚本与回退 prompt，支持多文件并发、超时回退、限流等待、断点续传和双语格式校验。
- 扩展凭据与 Python 缓存忽略规则：本地 `secrets_openai.txt*`、`__pycache__` 和字节码文件不会进入仓库。
